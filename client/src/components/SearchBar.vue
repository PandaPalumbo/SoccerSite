<template>
  <div class="position-relative w-100">
    ...
    />
<!--    TODO one search for leagues another to grab that leagues server once selected-->
      <!--    Player search bar-->
    <b-list-group class="position-absolute w-100 suggestions" v-if="focused && playerSuggestions && searchValue && dataType === 'players'">
      <div v-for="(item, i) in playerSuggestions" :key="i" >
          ...
          <b-row class="d-flex w-100 p-0 m-0">
            <b-col cols="1">
              <img class="league-img bg-light"
                   :src="`https://media.api-sports.io/football/players/${item.player_id}.png`"/>
            </b-col>
...
<!--              </p>-->
    ...
    <b-list-group class="position-absolute w-100 suggestions" v-else-if="focused && teamSuggestions && searchValue && dataType === 'teams'">
      <div v-for="(item, i) in teamSuggestions.slice(0, 10)" :key="i" >
        <b-list-group-item class="py-2 overflow-auto" href="#"
                           @mousedown.stop="addSelectedValue(item)" variant="dark">
          <b-row class="d-flex w-100 p-0 m-0">
            <b-col cols="1">
              <img class="league-img bg-light"
...
import {getPlayerInformation} from "@/api";
...
export default {
  ...
  methods: {
    addSelectedValue(item) {
      getPlayerInformation({id: item.player_id}, (data) => {
    ...
    async search(val) {
      ...
      if (val.length >= 4) {
        ...
        if(this.dataType === 'players')
          await this.$store.dispatch('queryPlayers', {
            query: val
          })
        if(this.dataType === 'leagues')
          await this.$store.dispatch('queryLeagues', {
            query: val
          })
