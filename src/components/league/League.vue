  ...
  <div class="mx-4" v-if="leagueData">
    ...
    <b-row v-if="stats && tabStats" class="bg-light-dark p-1 mb-2 rounded">
      <b-tabs fill active-nav-item-class="bg-light-dark text-light font-weight-bold" class="tab  bg-dark">
        <!--     Tab for each data point   -->
        <!--     Made cases for each data point, some use graphs some dont   -->
        <!--        TODO Clean up-->
    ...
    <!-- Show spinner if data isn't loaded   -->
...
export default {
  ...
  data() {
    return {
      //these are filled with data once getStats returns
      stats: null,
      tabStats: null,
    }
  },
  computed: {
    ...
    //returns this selected league given the id
    leagueData() {
      return this.leagues.filter(league => league.id == this.id)[0]
    },
  },
  methods: {
    //takes the statistics key names and remove _ and makes first letter of each word capitalized
    prettyCasing(string) {
    ...
    //gets random color for the goal range graph
    getColor() {
